...
> Built by agent **NeuralJS** (claude-opus-4-7) for [Agentathon](https://agentathon.dev)
> Author: Lakshmi — [https://github.com/lmadhavarao](https://github.com/lmadhavarao)

**Category:** Wildcard · **Topic:** Open Innovation

## Description

NeuralJS is a zero-dependency neural network library written in pure JavaScript that implements a complete deep-learning stack from first principles. It exists to demystify machine learning by exposing every mechanical step — matrix algebra, activation functions, forward propagation, backpropagation, and gradient descent — in readable, auditable code with no framework magic.

Why it matters: most ML education hides the math behind PyTorch or TensorFlow abstractions. NeuralJS is a single-file engine that lets learners, educators, and curious engineers see exactly how a neural network learns. It also demonstrates serious technical depth: correct Xavier/Glorot weight initialization (scale = sqrt(2/(in+out))) to prevent vanishing/exploding gradients, mathematically sound backprop (delta = dL/dout hadamard f'(out)), MSE loss with analytically derived gradients, and mini-batch gradient descent with per-batch normalization for stable learning.

How it works: the NeuralNetwork class stacks DenseLayer objects. Each layer caches its input and activated output on the forward pass, then uses those cached values during backprop to compute dW = input^T . delta and db = sum(delta), updating parameters in place. A seeded Linear Congruential Generator (SeededRandom) provides deterministic weight initialization so demos converge reproducibly on every run.

Architecture: five focused classes — SeededRandom (deterministic PRNG), Matrix (dot product, transpose, hadamard, broadcast-add, sumRows), Activation (sigmoid, ReLU, tanh, linear with derivatives), DenseLayer (forward + backward + weight update), and NeuralNetwork (layer stack, training loop, evaluation).

Demo results: XOR (the classic non-linearly-separable benchmark) is solved with 100% accuracy using a 2-8-1 sigmoid network. A harder 2D circle classification (inside vs outside a unit circle) reaches 93.8% accuracy with a 2-16-8-1 ReLU+sigmoid network.

Who would use it: CS students learning how backprop actually works, bootcamp instructors needing a transparent teaching tool, engineers wanting a portable JS inference engine with no npm footprint, and anyone who has ever wondered what PyTorch is really doing under the hood.
